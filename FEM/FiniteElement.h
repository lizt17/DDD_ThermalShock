...
//        /*                                  */ > ElementContainerType;
        ...
        const MeshType& mesh;
        std::map<typename SimplexTraits<_ElementType::dim,_ElementType::dim>::SimplexIDType,int> s2eID; // simplexID to elementID
        ...
        {/*!@param[in] s A const reference to a SimplicialMesh on which *this
          ...
          */
            ...
            int tempEleID(0);
            for (const auto& eleIter : elements() )
            {
                s2eID.insert(std::pair<typename SimplexTraits<_ElementType::dim,_ElementType::dim>::SimplexIDType,int>(eleIter.first,tempEleID));
                tempEleID++;
            }
        }
