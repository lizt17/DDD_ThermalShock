        ...
        void updateGeometry()
        {
            ...
            {// remove numerical errors by projecting along glidePlane->unitNormal
                ...
                if(this->links().size())
                {
                    ...
                    for(const auto& loopLink : this->links())
                    {
                        // 将滑移面积分割成三角形，S=1/2*ab*sin(theta)，但只有向量全同向时才能先计算矢量和，在计算模长
                ...
                //   法向量单位化
                const double nnDot(_rightHandedUnitNormal.dot(glidePlane->unitNormal));  // unitNormal为晶格？的单位法向量，此操作判断两者是否同向
