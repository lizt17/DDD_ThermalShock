...
//        typedef typename DislocationNetworkType::MatrixDim MatrixDim;
        ...
        typedef typename DislocationNetworkType::BvpAbaqusType::FiniteElementType FiniteElementType;
        typedef typename FiniteElementType::ElementType ElementType;
        typedef typename DislocationNetworkType::BvpAbaqusType::TrialFunctionType TrialFunctionType;
...
//        }
        ...
        {/*! Outputs DislocationNetwork data to the following files (x is the runID):
          */
            ...
            if(DN.outputMeshDisplacement)
            {
                ...
                {// add FEM solution and output
                    if( DN.simulationParameters.simulationType==DefectiveCrystalParameters::FINITE_FEM
                      || DN.simulationParameters.simulationType==DefectiveCrystalParameters::ABAQUS )
                    {
                        const size_t femID=DN.bvpAbaqus->finiteElement().mesh2femIDmap().at(node.pointID)->gID;
                        node+=DN.bvpAbaqus->displacement().dofs(femID);
                    }
            ...
            if (DN.bvpAbaqus && DN.outputFEMsolution )
            {
                if(!(runID%DN.bvpAbaqus->para.stepsBetweenBVPupdates))
                {// Output displacement and stress on external mesh faces
                    ...
                    u_file<<DN.bvpAbaqus->displacement().onBoundary();
                    model::cout<<magentaColor<<" ["<<(std::chrono::duration<double>(std::chrono::system_clock::now()-t0)).count()<<" sec]"<<defaultColor<<std::endl;
                    ...
                    model::SequentialOutputFile<'S',true> s_file_DD;
                    model::cout<<"		writing to S/S_"<<s_file.sID<<".txt"<<std::flush;
                    s_file<<DN.bvpAbaqus->stress().onBoundary();
            ...
            if(DN.bvpAbaqus) // output plastic strain increment, 
            {
                if (!((runID)%DN.bvpAbaqus->para.stepsBetweenBVPupdates))
                {
                    const auto tp=std::chrono::system_clock::now();
                    model::SequentialOutputFile<'P',1>::set_count(runID); // Vertices_file;
                    model::SequentialOutputFile<'P',1>::set_increment(DN.outputFrequency); // Vertices_file;
                    model::SequentialOutputFile<'P',true> p_file;
                    // UniqueOutputFile<'P'> p_file;
                    model::cout<<"		writing to P/P_"<<p_file.sID<<".txt"<<std::flush;
                    // model::cout<<" P/P_0.txt"<<std::flush;

                    

                    const size_t _elementSize(DN.bvpAbaqus->elementSize);
                    // Eigen::Matrix<double,Eigen::Dynamic,6> outPlasStrain(DN.bvpAbaqus->PlasticStrain());
                    // p_file<<"1 "<<std::endl<<DN.bvpAbaqus->PlasticStrain();
                    // p_file<<"2 "<<std::endl<<outPlasStrain; // in the order of ABAQUS element 
                    for (size_t k=0;k<_elementSize;k++)
                    {
                        // outPlasStrain.row(k)=DN.bvpAbaqus->PlasticStrain().row(DN.bvpAbaqus->IDmap.ElementMap()[k+1]);
                        // p_file<<std::setprecision(15)<<std::scientific<<DN.bvpAbaqus->PlasticStrain().row(DN.bvpAbaqus->IDmap.ElementMap()[k+1])<<std::endl;
                        p_file<<std::setprecision(15)<<std::scientific<<DN.bvpAbaqus->PlasticStrain().row(k)<<std::endl;
                        // p_file<<DN.bvpAbaqus->integralPoints.row(DN.bvpAbaqus->IDmap.ElementMap()[k+1])<<"  "<<DN.bvpAbaqus->integralPoints.row(DN.bvpAbaqus->IDmap.ElementMap()[k+1])<<std::endl;
                        // p_file<<DN.bvpAbaqus->PlasticStrain().row(k)<<std::endl;
                        //p_file<<k<<"  "<<DN.bvpAbaqus->IDmap.ElementMap()[k+1]<<std::endl;
                        //model::cout<<outPlasStrain.row(k);
                    }
                    
                    // for(int n=0;n<outPlasStrain.rows();n++)
                    // {
                    //     for(int j=0;j<outPlasStrain.cols();j++)
                    //     {
                    //         if (outPlasStrain(n,j)<0.0)
                    //         {
                    //             p_file<<" ";
                    //         }
                    //         else
                    ...
                    //             p_file<<"  ";
                    //         }
                    //         p_file<<std::setprecision(6)<<std::scientific<<outPlasStrain(n,j);
                    //     }
                    // model::cout<<"Size = "<<outPlasStrain.size()<<std::endl;
                        // p_file<<"3 "<<std::endl;
                        // p_file<<std::setprecision(15)<<std::scientific<<outPlasStrain;
                        // p_file<<std::endl<<runID<<std::endl;
                                        
                            
                    model::cout<<magentaColor<<" ["<<(std::chrono::duration<double>(std::chrono::system_clock::now()-tp)).count()<<" sec]"<<defaultColor<<std::endl;
                    DN.bvpAbaqus->clearPlasticStrain();
                    std::string fileDir("./flag/");
                    std::string fileName("flagP_"+std::to_string(runID)+".txt");
                    std::ofstream outFile(fileDir+fileName);
            ...
            if(DN.bvpAbaqus) // output plastic strain increment, 
            {
                if (!((runID)%DN.outputFrequency))
                {
                    ...
                    model::SequentialOutputFile<'N',1>::set_increment(DN.outputFrequency); // Vertices_file;
                    model::SequentialOutputFile<'N',true> n_file;
                    // UniqueOutputFile<'P'> p_file;
                    model::cout<<"		writing to N/N_"<<n_file.sID<<".txt"<<std::flush;
                    // model::cout<<" P/P_0.txt"<<std::flush;
                    for (size_t nk=0;nk<DN.NucSite.size();nk++)
                    {
                    n_file<<std::setprecision(15)<<std::scientific<<DN.NucSite[nk].transpose()<<std::endl;
                    }
                    //DN.NucSite.clear();
                }
            }
