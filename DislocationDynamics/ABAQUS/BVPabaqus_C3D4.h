...
//            const double  nu=Material<dim,Isotropic>::nu;
    ...
    public:
        ...
        Eigen::VectorXd stressV; // voigt stress in all node, ID is form Abaqus
       ...
       /* init  */,nc(TextFileParser("./inputFiles/DD.txt").readScalar<double>("g_nc",true))
...

        {
            ...
            elementSize = TrialBase<TrialFunctionType>::elementSize();
        ...
        void calNodalStress(const long int& runID)
        {
            ...
            std::string fileNameFlag("flagS_"+std::to_string(runID)+".txt");
            std::string fileNameS("stress_"+std::to_string(runID)+".txt");
