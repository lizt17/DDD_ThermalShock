                ...
                if(parentSegment.network().computeDDinteractions)
                {
                    ...
                    for(const auto& link : parentSegment.network().links())
                    {
                        if(   !link.second->hasZeroBurgers()
                          && !(link.second->isBoundarySegment() && parentSegment.network().simulationParameters.simulationType==DefectiveCrystalParameters::FINITE_NO_FEM) 
                        //    &&  link.second->isBoundarySegment()  // exclude boundary segments even if they are non-zero Burgers && parentSegment.network().simulationParameters.simulationType==DefectiveCrystalParameters::FINITE_NO_FEM)
