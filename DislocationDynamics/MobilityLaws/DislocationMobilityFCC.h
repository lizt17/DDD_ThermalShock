...
namespace model
{
    ...
    struct DislocationMobilityFCC : public DislocationMobilityBase
    {
        ...
        //      double B_eT = B0e+B1e*T;
        //      double B_sT = B0s+B1s*T;
        //     double vve=std::fabs(b.transpose()*S*n)/(B_eT);
        //     double tau =  std::fabs(b.transpose()*S*n);

        //    double ve=1/(2*vve)*(std::sqrt(1+4*vve*vve)-1);
        //    //std::cout<<"ve="<<ve <<std::endl;
        //     double vvs=std::fabs(b.transpose()*S*n)/(B0s+B1s*T);
            
        //     double vs=1/(2*vvs)*(std::sqrt(1+4*vvs*vvs)-1);
