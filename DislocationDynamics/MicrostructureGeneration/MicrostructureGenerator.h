 ...
 */
/*
  * 2021-7-24, lizt  addIndividualFrankReadSources
  *
  */
...
namespace model
{
    ...
    class MicrostructureGenerator
    {
            ...
            if(targetStraightDislocationDensity>0.0)
            {
                ...
                while(density<targetStraightDislocationDensity)
                {
                       ...
                       && ((fabs(helicity+dh)>fabs(helicity) && fabs(dh)>FLT_EPSILON ) || helicity==0.0 || !enforceMonotonicHelicity))
                    {
                        ...
                        // write loop file
                        configIO.loops().emplace_back(loopID+0, b,n,P0,grainID,DislocationLoopIO<dim>::GLISSILELOOP,-1,VectorDimD::Zero());
        ...
        // 2021-7-23, lizt
        /**********************************************************************/
        void addIndividualFrankReadSources()
        {
            if(individualFrankReadSourcesSlipSystemIDs.size())
            {
                std::cout<<magentaBoldColor<<"Generating Individual Frank-Read Sources"<<defaultColor<<std::endl;
                if(individualFrankReadSourcesSlipSystemIDs.size()!=pointsAlongFrankReadSources.rows())
                {
                    std::cout<<"individualFrankReadSourcesSlipSystemIDs.size() = "<<individualFrankReadSourcesSlipSystemIDs.size()<<std::endl;
                    std::cout<<"pointsAlongFrankReadSources.size() = "<<pointsAlongFrankReadSources.size()<<std::endl;
                    std::cout<<"You must provide a point for each individual Frank-Read source dislocation. Exiting. "<<std::endl;
                    exit(EXIT_FAILURE);
                }

                for(size_t k=0;k<individualFrankReadSourcesSlipSystemIDs.size();k++)
                {
                    
                    const int& rSS(individualFrankReadSourcesSlipSystemIDs[k]);
                    
                    if(rSS>=0)
                    {
                        std::pair<bool,const Simplex<dim,dim>*> found=mesh.search(pointsAlongFrankReadSources.row(k));
                        if(!found.first)
                        {
                            std::cout<<"Point "<<pointsAlongFrankReadSources.row(k)<<" is outside mesh. EXITING. "<<std::endl;
                            exit(EXIT_FAILURE);
                        }

                        auto P_in=pointsAlongFrankReadSources.row(k);                        
                        const int grainID=found.second->region->regionID;
                        const LatticeVector<dim> LP = poly.grain(grainID).snapToLattice(P_in);                        
                        auto searchResult = mesh.searchRegionWithGuess(LP.cartesian(),found.second);
                        if(!searchResult.first)
                        {
                            std::cout<<"point "<<P_in<<" is outside mesh, please change the position of point. "<<std::endl;
                            exit(EXIT_FAILURE);
                        }


                        std::cout<<"generating individual Frank-Read Sources "<<k<<defaultColor<<std::endl;
                        ...
                        if(rSS>=int(poly.grain(grainID).slipSystems().size()))
                        {
                            ...
                            std::cout<<"requested slip system ID does not exsit. EXITING. "<<std::endl;
                            exit(EXIT_FAILURE);
                        }
                        ...
                        VectorDimD b(slipSystem.s.cartesian());
                        VectorDimD n=slipSystem.unitNormal; // slip plane normal


                        std::uniform_real_distribution<> dis(0.0, 0.5*M_PI);
                        const double theta=dis(generator); // random angle of the dislocation line in the plane from screw orientation.
                        VectorDimD line1=Eigen::AngleAxisd(90.0, n)*b.normalized();
                        // VectorDimD line2=Eigen::AngleAxisd(theta+0.5*M_PI, n)*b.normalized();

                        // ReciprocalLatticeDirection<3> sr(poly.grain(grainID).reciprocalLatticeDirection(line1));

                        // bool isEdge=true;
                        // LatticeDirection<3> d1(LatticeVector<dim>(sr.cross(slipSystem.n)*randomSign()));
                        LatticeDirection<3> d1 = poly.grain(grainID).snapToLattice(line1);
                        double d1cNorm(d1.cartesian().norm());

                        int a1 = FrankReadSourceLenth[k]/d1cNorm/2;
                        std::cout<<"a1 = "<<a1<<std::endl;
                        
                        LatticeVector<dim> L0=LP-d1*a1;
                        LatticeVector<dim> L1=LP+d1*a1;

                        // Compute the LatticeDirection corresponding to n
                        LatticeDirection<3> d2(poly.grain(grainID).latticeDirection(-slipSystem.n.cartesian()));
                        if(rSS>=16 && rSS<=19)
                        {
                            d2=(poly.grain(grainID).latticeDirection( slipSystem.n.cartesian()));
                            b=-b;
                        }
                        // LatticeDirection<3> d2(poly.grain(grainID).latticeDirection(-slipSystem.n.cartesian()*randomSign()));
                        double d2cNorm(d2.cartesian().norm());

                        const int a2=a1*2*individualFrankReadAspectRatio[k];
                        std::cout<<"  = "<<individualFrankReadAspectRatio[k]<<std::endl;
...
//                        PlaneMeshIntersectionContainerType pmi01=PlaneMeshIntersection<dim>(mesh,P0,n2,grainID);
                        ...
                        if ((P1-P0).norm()>a2*0.5 && (P3-P2).norm()>a2*0.5)  //not too small arm
                        {
                            ...
                            configIO.loops().emplace_back(loopID,slipSystem.s.cartesian(),n2,P0,grainID,DislocationLoopIO<dim>::GLISSILELOOP,-1,VectorDimD::Zero());
                            ...
                            configIO.loops().emplace_back(loopID+1,slipSystem.s.cartesian(),n1,P0,grainID,DislocationLoopIO<dim>::SESSILELOOP,-1,VectorDimD::Zero());
                            ...
                            configIO.loops().emplace_back(loopID+2,slipSystem.s.cartesian(),n1,P3,grainID,DislocationLoopIO<dim>::GLISSILELOOP,-1,VectorDimD::Zero());
                ...
                while(density<targetPrismaticLoopDensity)
                {
                       ...
                       && ((fabs(helicity+dh)>fabs(helicity) && fabs(dh)>FLT_EPSILON ) || helicity==0.0 || !enforceMonotonicHelicity))
                    {
                        ...
                        for(size_t k=0;k<posVector.size();++k)
                        {
                            ...
                            configIO.loops().emplace_back(loopID+k,b,normalsVector[k],posVector[k],grainID,DislocationLoopIO<dim>::GLISSILELOOP,-1,VectorDimD::Zero());
                        ...
                        configIO.loops().emplace_back(loopID+posVector.size(),-b,b.normalized(),posVector[0],grainID,DislocationLoopIO<dim>::SESSILELOOP,-1,VectorDimD::Zero());
...
//        const double averageLoopSize;
        ...
        // 2021-7-23, lizt
        // individual Frank-Read sources
        const std::vector<int> individualFrankReadSourcesSlipSystemIDs;
        const std::vector<double> FrankReadSourceLenth;
        const std::vector<double> individualFrankReadAspectRatio;
        const Eigen::Matrix<double,Eigen::Dynamic,dim> pointsAlongFrankReadSources;
...
//        /* init*/,meshID(TextFileParser("./inputFiles/DD.txt").readScalar<int>("meshID",true))
        ...
        /* init*/,individualFrankReadSourcesSlipSystemIDs(TextFileParser("./inputFiles/initialMicrostructure.txt").readArray<int>("individualFrankReadSourcesSlipSystemIDs",true))
        /* init*/,FrankReadSourceLenth(TextFileParser("./inputFiles/initialMicrostructure.txt").readArray<double>("FrankReadSourceLenth",true))
        /* init*/,individualFrankReadAspectRatio(TextFileParser("./inputFiles/initialMicrostructure.txt").readArray<double>("individualFrankReadAspectRatio",true))
        /* init*/,pointsAlongFrankReadSources(TextFileParser("./inputFiles/initialMicrostructure.txt").readMatrix<double>("pointsAlongFrankReadSources",individualFrankReadSourcesSlipSystemIDs.size(),dim,true))
        {
            ...
            if(targetSFTdensity>0.0)
            {
                ...
                if(poly.crystalStructure=="FCC")
                {
                       ...
                       && mesh.searchRegion(grainID,P2).first)
                    {
                        ...
                        configIO.loops().emplace_back(loopID+0, b,a2.cross(a1).cartesian(),P0,grainID,DislocationLoopIO<dim>::GLISSILELOOP,-1,VectorDimD::Zero());
                        ...
                        configIO.loops().emplace_back(loopID+1, b,a1.cross(a3).cartesian(),P0,grainID,DislocationLoopIO<dim>::GLISSILELOOP,-1,VectorDimD::Zero());
                        ...
                        configIO.loops().emplace_back(loopID+2, b,a3.cross(a2).cartesian(),P0,grainID,DislocationLoopIO<dim>::GLISSILELOOP,-1,VectorDimD::Zero());
                        ...
                        configIO.loops().emplace_back(loopID+3, b,a31.cross(a12).cartesian(),P0,grainID,DislocationLoopIO<dim>::GLISSILELOOP,-1,VectorDimD::Zero());
